...
from models.models import (
    ...
    EventAttendance, AttendanceCreate, AttendanceRead, AttendanceReadWithMember,
    Member,
)
...
def create_event(
    ...
    sync_with_chatbot(event.title, event.description or "Nouvel événement du club", category="event")
...
@router.get("/{event_id}/attendees", response_model=List[AttendanceReadWithMember],
            summary="See who is attending (Head only)")
def event_attendees(event_id: int, session: Session = Depends(get_session),
                    _: Member = Depends(require_mod_or_head)): # Mod or Head can see attendees
    return session.exec(
